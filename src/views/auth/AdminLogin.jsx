...
const AdminLogin = () => {
    ...
    useEffect(() => {
        if (errorMessage) {
            toast.error(errorMessage, {position: 'top-right', style: {background: '#333', color: '#fff'}});
            dispatch(messageClear());
        }
        if (successMessage) {
            toast.success(successMessage, {position: 'top-right', style: {background: '#333', color: '#fff'}});
            dispatch(messageClear());
            navigate('/');
        }
                ...
                <div className="bg-[#6f68d1] p-6 rounded-md">
                    <div className="h-[70px] flex justify-center items-center">
                        <div className="w-[180px] h-[50px]">
                            <img className="w-full h-full" src="http://localhost:3000/images/logo.png" alt="site logo" />
                        </div>
                    </div>
